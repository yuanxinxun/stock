...
stock_dict=stock_basic.get_datadict(stock_code_namedict.keys(),'D:\python_project\source_data\data',userows=30,usecols=[0,2],columns=['close'])
    ...
    if is_notrun_slope or inputstr[0]=="r":
        ...
        for code,value_df in industry_dict.items():
            ...
            if amt_avr<use_trade_minamt:#滤掉平均成交量过低的板块
                log("忽略%s板块%s，平均成交额%.2f亿"%(code,code_namedict[code],amt_avr/100000000))
                continue
            ...
            slope_df.loc[code]=[code_namedict[code],slope]
        slope_df=slope_df.sort_values(by=["slope"],ascending=False)
    if inputstr[0]=="r":#为了is_notrun_slope=True的时候也能正常执行，保持程序丝滑
        pass
    elif len(inputstr)==6:
        errcode,windtp=w.wset("sectorconstituent",windcode=inputstr+".SI",usedf=True)
